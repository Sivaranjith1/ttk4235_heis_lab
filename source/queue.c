...
FloorOrder *queue_get_next_floor_order(uint8_t current_floor, QueueDirection current_direction){
    ...
    while(++i < MAX_ITERATION)
    {
      if(current_direction == QUEUE_DIRECTION_DOWN){
        if((current_list_element->toFloor < current_floor && current_list_element->toFloor >= closet_floor_element->toFloor && current_list_element->direction != DIRECTION_UP) || closet_floor_element->toFloor >= current_floor || (closet_floor_element->direction == DIRECTION_UP && current_list_element->direction != DIRECTION_UP)){
